...
*备    注:
********************************************************************************/
__s32 check_magic( char *mem_base, const char *magic )
{
	struct spare_boot_ctrl_head  *bfh;
	unsigned char *p;
  bfh = (struct spare_boot_ctrl_head*)get_spare_head(mem_base);
	p = bfh->magic;
...
*备    注:
********************************************************************************/
__s32 check_sum( char *mem_base, __u32 size )
{
	__u32 *buf;
	__u32 count;
	__u32 src_sum;
...
*备    注:
********************************************************************************/
__s32 check_file(char *mem_base, __u32 size, const char *magic )
{
...
__s32 gen_check_sum( char *boot_buf )
{
	...
	head_p = (struct spare_boot_head_t *)get_spare_head(boot_buf);
	length = head_p->boot_head.length;

//	printf("length=%d\n", length);
	if( ( length & 0x3 ) != 0 )                   // must 4-byte-aligned
	{
	  printf("%s: file not aligned!\n",__FILE__);
		return -1;
	}
	buf = (__u32 *)boot_buf;

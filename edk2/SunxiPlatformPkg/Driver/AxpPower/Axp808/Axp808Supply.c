...
static int Axp808_set_dcdc1(int set_vol, int onoff)
{
  ...
  if(set_vol > 0)
  {
    ...
    if(Axp808PmBusRead(AXP808_ADDR, BOOT_POWER808_DCAOUT_VOL, &reg_value))
    {
      return EFI_DEVICE_ERROR;
    }
    reg_value &= 0x80;
    reg_value |= tmp_step;
    if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_DCAOUT_VOL, reg_value))
    {
  ...
  if(Axp808PmBusRead(AXP808_ADDR, BOOT_POWER808_OUTPUT_CTL1, &reg_value))
  {
    return EFI_DEVICE_ERROR;
  }
  if(onoff == 0)
  ...
  if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_OUTPUT_CTL1, reg_value))
  {
...
static int Axp808_set_dcdc2(int set_vol, int onoff)
{
  ...
  if(set_vol > 0)
  {
    ...
    if(Axp808PmBusRead(AXP808_ADDR, BOOT_POWER808_DCBOUT_VOL, &reg_value))
    {
      return EFI_DEVICE_ERROR;
    }
    reg_value &= ~0x1f;
    reg_value |= (set_vol - 1000)/50;
    if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_DCBOUT_VOL, reg_value))
    {
...
static int Axp808_set_dcdc3(int set_vol, int onoff)
{
  ...
  if(set_vol > 0)
  {
    ...
    if(Axp808PmBusRead(AXP808_ADDR, BOOT_POWER808_DCCOUT_VOL, &reg_value))
    {
      return EFI_DEVICE_ERROR;
    }
    reg_value &= 0x80;
    reg_value |= tmp_step;
    if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_DCCOUT_VOL, reg_value))
    {
...
static int Axp808_set_dcdc4(int set_vol, int onoff)
{
  ...
  if(set_vol > 0)
  {
    ...
    if(Axp808PmBusRead(AXP808_ADDR, BOOT_POWER808_DCDOUT_VOL, &reg_value))
    {
      return EFI_DEVICE_ERROR;
    }
    reg_value &= 0xC0;
    ...
    if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_DCDOUT_VOL, reg_value))
    {
...
static int Axp808_set_dcdc5(int set_vol, int onoff)
{
  ...
  if(set_vol > 0)
  {
    ...
    if(Axp808PmBusRead(AXP808_ADDR, BOOT_POWER808_DCEOUT_VOL, &reg_value))
    {
      return EFI_DEVICE_ERROR;
    }
    reg_value &= 0xC0;
    reg_value |= ((set_vol - 1100)/100);
    if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_DCEOUT_VOL, reg_value))
    {
...
static int Axp808_set_aldo1(int set_vol, int onoff)
{
  ...
  if(set_vol > 0)
  {
    ...
    if(Axp808PmBusRead(AXP808_ADDR, BOOT_POWER808_ALDO1OUT_VOL, &reg_value))
    {
      return EFI_DEVICE_ERROR;
    }
    reg_value &= 0xE0;
    reg_value |= ((set_vol - 700)/100);
    if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_ALDO1OUT_VOL, reg_value))
    {
...
static int Axp808_set_aldo2(int set_vol, int onoff)
{
  ...
  if(set_vol > 0)
  {
    ...
    if(Axp808PmBusRead(AXP808_ADDR, BOOT_POWER808_ALDO2OUT_VOL, &reg_value))
    {
      return EFI_DEVICE_ERROR;
    }
    reg_value &= 0xE0;
    reg_value |= ((set_vol - 700)/100);
    if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_ALDO2OUT_VOL, reg_value))
    {
...
static int Axp808_set_aldo3(int set_vol, int onoff)
{
  ...
  if(set_vol > 0)
  {
    ...
    if(Axp808PmBusRead(AXP808_ADDR, BOOT_POWER808_ALDO3OUT_VOL, &reg_value))
    {
      return EFI_DEVICE_ERROR;
    }
    reg_value &= 0xE0;
    reg_value |= ((set_vol - 700)/100);
    if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_ALDO3OUT_VOL, reg_value))
    {
...
static int Axp808_set_bldo1(int set_vol, int onoff)
{
  ...
  if(set_vol > 0)
  {
    ...
    if(Axp808PmBusRead(AXP808_ADDR, BOOT_POWER808_BLDO1OUT_VOL, &reg_value))
    {
      return EFI_DEVICE_ERROR;
    }
    reg_value &= 0xf0;
    reg_value |= ((set_vol - 700)/100);
    if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_BLDO1OUT_VOL, reg_value))
    {
  ...
  if(Axp808PmBusRead(AXP808_ADDR, BOOT_POWER808_OUTPUT_CTL2, &reg_value))
  {
    return EFI_DEVICE_ERROR;
  }
  if(onoff == 0)
  ...
  if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_OUTPUT_CTL2, reg_value))
  {
...
static int Axp808_set_bldo2(int set_vol, int onoff)
{
  ...
  if(set_vol > 0)
  {
    ...
    if(Axp808PmBusRead(AXP808_ADDR, BOOT_POWER808_BLDO2OUT_VOL, &reg_value))
    {
      return EFI_DEVICE_ERROR;
    }
    reg_value &= 0xf0;
    reg_value |= ((set_vol - 700)/100);
    if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_BLDO2OUT_VOL, reg_value))
    {
      DEBUG((EFI_D_ERROR,"sunxi pmu error : unable to set bldo1\n"));
      return EFI_DEVICE_ERROR;
    }
...
static int Axp808_set_bldo3(int set_vol, int onoff)
{
  ...
  if(set_vol > 0)
  {
    ...
    if(Axp808PmBusRead(AXP808_ADDR, BOOT_POWER808_BLDO3OUT_VOL, &reg_value))
    {
      return EFI_DEVICE_ERROR;
    }
    reg_value &= 0xf0;
    reg_value |= ((set_vol - 700)/100);
    if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_BLDO3OUT_VOL, reg_value))
    {
      DEBUG((EFI_D_ERROR,"sunxi pmu error : unable to set bldo1\n"));
      return EFI_DEVICE_ERROR;
    }
...
static int Axp808_set_bldo4(int set_vol, int onoff)
{
  ...
  if(set_vol > 0)
  {
    ...
    if(Axp808PmBusRead(AXP808_ADDR, BOOT_POWER808_BLDO4OUT_VOL, &reg_value))
    {
      return EFI_DEVICE_ERROR;
    }
    reg_value &= 0xf0;
    reg_value |= ((set_vol - 700)/100);
    if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_BLDO4OUT_VOL, reg_value))
    {
      DEBUG((EFI_D_ERROR,"sunxi pmu error : unable to set bldo1\n"));
      return EFI_DEVICE_ERROR;
    }
...
static int Axp808_set_cldo1(int set_vol, int onoff)
{
  ...
  if(set_vol > 0)
  {
    ...
    if(Axp808PmBusRead(AXP808_ADDR, BOOT_POWER808_CLDO1OUT_VOL, &reg_value))
    {
      return EFI_DEVICE_ERROR;
    }
    reg_value &= 0xf0;
    reg_value |= ((set_vol - 700)/100);
    if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_CLDO1OUT_VOL, reg_value))
    {
      DEBUG((EFI_D_ERROR,"sunxi pmu error : unable to set bldo1\n"));
      return EFI_DEVICE_ERROR;
    }
...
static int axp808_set_swout(int set_vol, int onoff)
{
  ...
  if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_OUTPUT_CTL2, reg_value))
  {
    DEBUG((EFI_D_ERROR,"sunxi pmu error : unable to onoff swout\n"));
    return EFI_DEVICE_ERROR;
  }
  return EFI_SUCCESS;
}
...
static int Axp808_set_cldo2(int set_vol, int onoff)
{
  ...
  if(set_vol > 0)
  {
    ...
    if(Axp808PmBusRead(AXP808_ADDR, BOOT_POWER808_CLDO2OUT_VOL, &reg_value))
    {
      return EFI_DEVICE_ERROR;
    }
    reg_value &= 0xf0;
    reg_value |= ((set_vol - 700)/100);
    if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_CLDO2OUT_VOL, reg_value))
    {
      DEBUG((EFI_D_ERROR,"sunxi pmu error : unable to set bldo1\n"));
      return EFI_DEVICE_ERROR;
    }
...
static int Axp808_set_cldo3(int set_vol, int onoff)
{
  ...
  if(set_vol > 0)
  {
    ...
    if(Axp808PmBusRead(AXP808_ADDR, BOOT_POWER808_CLDO3OUT_VOL, &reg_value))
    {
      return EFI_DEVICE_ERROR;
    }
    reg_value &= 0xf0;
    reg_value |= ((set_vol - 700)/100);
    if(Axp808PmBusWrite(AXP808_ADDR, BOOT_POWER808_CLDO3OUT_VOL, reg_value))
    {
      DEBUG((EFI_D_ERROR,"sunxi pmu error : unable to set bldo1\n"));
      return EFI_DEVICE_ERROR;
    }
...
EFI_STATUS Axp808SetSupplyStatus(IN UINTN VoltageIndex, IN UINTN Voltage, IN INTN OnOff)
{
  int supply_type;
  int sppply_index;
  ...
  switch(supply_type)
  {
    case PMU_SUPPLY_DCDC_TYPE:
      return Axp808_set_dcdc_output(sppply_index, Voltage, OnOff);

    case PMU_SUPPLY_ADLO_TYPE:
      return Axp808_set_aldo_output(sppply_index, Voltage, OnOff);

    case PMU_SUPPLY_BDLO_TYPE:
      return Axp808_set_bldo_output(sppply_index, Voltage, OnOff);

    case PMU_SUPPLY_CDLO_TYPE:
      return Axp808_set_cldo_output(sppply_index, Voltage, OnOff);
...
EFI_STATUS Axp808SetSupplyStatusByName(IN CHAR8 *VoltageName,IN UINTN Voltage, IN INTN OnOff)
{
  int sppply_index;
  ...
  else if(!AsciiStrnCmp(VoltageName, "cldo", 4))
  {
    sppply_index = AsciiStrDecimalToUintn(VoltageName + 4);
    return Axp808_set_cldo_output(sppply_index, Voltage, OnOff);
  }else if (!AsciiStrnCmp(VoltageName, "swin", 4) || !AsciiStrnCmp(VoltageName, "sw0", 3))
  {
    return axp808_set_swout(Voltage, OnOff);
  }
...
EFI_STATUS Axp808ProbeSupplyStatus(IN UINTN VoltageIndex,IN UINTN  *Voltage, IN INTN *OnOff)
{
  return EFI_UNSUPPORTED;
}
...
static EFI_STATUS axp808_probe_swout_output(int sppply_index)
{
  switch(sppply_index)
  {
    case 1:
...
EFI_STATUS axp808_probe_supply_status(int vol_name, int vol_value, int onoff)
{
  return 0;
}
EFI_STATUS Axp808ProbeSupplyStatusByName(IN CHAR8  *vol_name, UINTN *Vol)
{
  int sppply_index;
  int ret = -1;
  ...
  *Vol = 0;
  if(!AsciiStrnCmp(vol_name, "dcdc", 4))
  {
    ...
    ret =  Axp808_probe_dcdc_output(sppply_index);
  }
  else if(!AsciiStrnCmp(vol_name, "aldo", 4))
  {
    sppply_index = 1 + vol_name[4] - '1';
    ret = Axp808_probe_aldo_output(sppply_index);
  }
  else if(!AsciiStrnCmp(vol_name, "bldo", 4))
  {
    sppply_index = 1 + vol_name[4] - '1';
    ret =  Axp808_probe_bldo_output(sppply_index);
  }
  else if(!AsciiStrnCmp(vol_name, "cldo", 4))
  {
    sppply_index = 1 + vol_name[4] - '1';
    ret= Axp808_probe_cldo_output(sppply_index);
  }
  else if (!AsciiStrnCmp(vol_name, "swin", 4) || !AsciiStrnCmp(vol_name, "sw0", 3))
  {
    sppply_index = 1;

...
static EFI_STATUS Axp806LibPmBusRead(UINT8 DeviceRegister, UINT8 *Buffer)
{
  EFI_STATUS               Status;
  UINTN                    DeviceBufferLength = 1 + 1 + 1; //deviceid + device address + data
  UINT8                    DeviceBuffer[3];

  DeviceBuffer[0] = RSB_RTSADDR_AXP806;
...
static EFI_STATUS Axp806LibPmBusWrite(UINT8 DeviceRegister, UINT8 data)
{
  EFI_STATUS               Status;
  UINTN                    DeviceBufferLength = 1 + 1 + 1; //deviceid + device address + data
  UINT8                    DeviceBuffer[3];
...
.AxpPmBusRead  = Axp806LibPmBusRead,
.AxpPmBusWrite = Axp806LibPmBusWrite
};

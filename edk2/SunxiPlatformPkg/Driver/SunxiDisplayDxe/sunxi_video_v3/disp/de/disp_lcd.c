...
static s32 lcd_clk_config(struct disp_device* lcd)
{
  ...
  unsigned int pll_rate = 297000000, lcd_rate = 33000000, dclk_rate = 33000000, dsi_rate = 0;//hz
  unsigned int pll_rate_set = 297000000, lcd_rate_set = 33000000, dclk_rate_set = 33000000, dsi_rate_set = 0;//hz
  ...
  if ((pll_rate_set != pll_rate) || (lcd_rate_set != lcd_rate)
    || (dclk_rate_set != dclk_rate)) {
      DE_WRN("disp %d, clk: pll(%d),clk(%d),dclk(%d) dsi_rate(%d)\n     clk real:pll(%d),clk(%d),dclk(%d) dsi_rate(%d)\n",
...
static s32 disp_lcd_pin_cfg(struct disp_device *lcd, u32 bon)
{
  ...
  DE_INF("lcd %d pin config, state %a, %d\n", lcd->disp, (bon)? "on":"off", bon);
...
//FIXME
extern UINT32 disp_sys_get_irq_para(u32 idnum);
...
#endif
{
  struct disp_device *lcd = NULL;//(struct disp_device*)parg;
  struct disp_lcd_private_data *lcdp = NULL;
  struct disp_manager *mgr = NULL;
  u32 hwdev_index;
  
  lcd =  (struct disp_device*)disp_sys_get_irq_para(Source);
  //__wrn("++++++++%a call, irq=%d, pArg = %x\n", __func__,(u32)Source,(u32)lcd );
  if (NULL == lcd)
...
static s32 disp_lcd_set_panel_funs(struct disp_device* lcd, char *name, disp_lcd_panel_fun * lcd_cfg)
{
  ...
  DE_INF("lcd %d, driver_name %a,  panel_name %a\n", lcd->disp, drv_name, name);

            ...
            unsigned char celut[256], hist_data* p_hist_data)
{
  ...
  else
  {
    ...
    {
      printf("p_hist_data->white_thr0(%d) >= 256", p_hist_data->white_thr0);
      while(1);
    }
...
//  weight = 256;
  ...
  {
    __wrn("%a, ##################error para, mean=%d, validsum=%d, validcnt=%d, pd_ymin_fix=%d, pd_black=%d, pd_white=%d, pd_ymax_fix=%d,sum=%d\n",

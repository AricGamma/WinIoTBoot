...
s32 lcd_parse_panel_para(u32 screen_id, disp_panel_para * info)
{
    ...
    AsciiSPrint(primary_key,sizeof(primary_key), "lcd%d_para", screen_id);
...
//lcd_used
    ...
    ZeroMem(info, sizeof(disp_panel_para));
...
//lcd_bl_en
  ...
  AsciiSPrint(sub_name,sizeof(sub_name),"lcd_bl_regulator");
...
//lcd_power0
  for(i=0; i<LCD_POWER_NUM; i++)
  {
    ...
    if(i==0)
      AsciiSPrint(sub_name, sizeof(sub_name),"lcd_power");
    else
      AsciiSPrint(sub_name, sizeof(sub_name),"lcd_power%d", i);
    lcd_cfg->lcd_power_type[i] = 0;
        ZeroMem(tmp_val,sizeof(tmp_val));
    ret = OSAL_Script_FetchParser_Data_Ex(primary_key,sub_name, (int *)(tmp_val), &type, 25);
        AsciiStrCpy(lcd_cfg->lcd_regu[i],tmp_val);
    if(ret == 3) {
...
//lcd_gpio
    for(i=0; i<4; i++)
    {
        AsciiSPrint(sub_name,sizeof(sub_name), "lcd_gpio_%d", i);
...
//lcd_gpio_scl,lcd_gpio_sda
  ...
  {
    AsciiSPrint(sub_name,sizeof(sub_name), "lcd_gpio_regulator%d", i);
...
//lcd io
  ...
  AsciiSPrint(sub_name,sizeof(sub_name), "lcd_io_regulator");
  ret = OSAL_Script_FetchParser_Data(primary_key, sub_name, (int *)lcd_cfg->lcd_io_regulator, 25);

//backlight adjust
  for(i = 0; i < 101; i++) {
    AsciiSPrint(sub_name,sizeof(sub_name), "lcd_bl_%d_percent", i);
...
//init_bright
    AsciiSPrint(primary_key, sizeof(primary_key),"disp_init");
    AsciiSPrint(sub_name,sizeof(sub_name), "lcd%d_backlight", screen_id);
...
//bright,constraction,saturation,hue
    AsciiSPrint(primary_key, sizeof(primary_key), "disp_init");
    AsciiSPrint(sub_name,sizeof(sub_name), "lcd%d_bright", screen_id);
    ret = OSAL_Script_FetchParser_Data(primary_key, sub_name, &value, 1);
    if(ret < 0)
    {
    ...
    AsciiSPrint(sub_name,sizeof(sub_name), "lcd%d_contrast", screen_id);
    ret = OSAL_Script_FetchParser_Data(primary_key, sub_name, &value, 1);
    if(ret < 0)
    {
    ...
    AsciiSPrint(sub_name,sizeof(sub_name), "lcd%d_saturation", screen_id);
    ret = OSAL_Script_FetchParser_Data(primary_key, sub_name, &value, 1);
    if(ret < 0)
    {
    ...
    AsciiSPrint(sub_name,sizeof(sub_name), "lcd%d_hue", screen_id);
    ret = OSAL_Script_FetchParser_Data(primary_key, sub_name, &value, 1);
    if(ret < 0)
    {
        lcd_cfg->lcd_hue = 50;
...
s32 disp_lcd_pin_cfg(struct disp_lcd *lcd, u32 bon)
{
  ...
  //io-pad
  if(bon == 1) {
    if(!((!AsciiStrCmp(lcdp->lcd_cfg.lcd_io_regulator, "")) || (!AsciiStrCmp(lcdp->lcd_cfg.lcd_io_regulator, "none"))))
  ...
  for(i=0; i<28; i++) {
    if(lcdp->lcd_cfg.lcd_io_used[i]) {
      ...
      CopyMem(gpio_info, &(lcdp->lcd_cfg.lcd_io[i]), sizeof(disp_gpio_set_t));
      if(!bon) {
        gpio_info->mul_sel = 7;
      } else {
...
s32 disp_lcd_get_driver_name(struct disp_lcd *lcd, char* name)
{
  ...
  AsciiSPrint(primary_key,sizeof(primary_key), "lcd%d_para", lcd->channel_id);
...
s32 disp_lcd_set_panel_funs(struct disp_lcd* lcd, disp_lcd_panel_fun * lcd_cfg)
{
  ...
  ZeroMem(&lcdp->lcd_panel_fun, sizeof(disp_lcd_panel_fun));
...
s32 disp_lcd_backlight_enable(struct disp_lcd *lcd)
{
  ...
  //io-pad
  if(!((!AsciiStrCmp(lcdp->lcd_cfg.lcd_bl_regulator, "")) || (!AsciiStrCmp(lcdp->lcd_cfg.lcd_bl_regulator, "none"))))
  ...
  if(disp_lcd_is_used(lcd) && (0 < lcdp->lcd_cfg.backlight_bright)) {
    if(lcdp->lcd_cfg.lcd_bl_en_used) {
      CopyMem(gpio_info, &(lcdp->lcd_cfg.lcd_bl_en), sizeof(disp_gpio_set_t));
...
s32 disp_lcd_power_enable(struct disp_lcd *lcd, u32 power_id)
{
  ...
  if(disp_lcd_is_used(lcd)) {
    if(lcdp->lcd_cfg.lcd_power_type[power_id] == 1) {
      CopyMem(gpio_info, &(lcdp->lcd_cfg.lcd_power[power_id]), sizeof(disp_gpio_set_t));
...
s32 disp_lcd_get_timing(struct disp_lcd *lcd, disp_video_timing * tt)
{
  ...
  ZeroMem(tt, sizeof(disp_video_timing));
...
s32 disp_lcd_get_panel_info(struct disp_lcd *lcd, disp_panel_para* info)
{
  ...
  CopyMem(info, (disp_panel_para*)(&(lcdp->panel_info)), sizeof(disp_panel_para));
  return 0;
}
...
#else
VOID EFIAPI disp_lcd_event_proc(
...
void disp_lcd_event_fun(u32 screen_id)
{
    static u32 cntr=0;
...
VOID EFIAPI disp_lcd0_event_proc(
  IN  HARDWARE_INTERRUPT_SOURCE   Source,
  IN  EFI_SYSTEM_CONTEXT          SystemContext     
  )

{
    disp_lcd_event_fun(0);
}

VOID EFIAPI disp_lcd1_event_proc (
  IN  HARDWARE_INTERRUPT_SOURCE   Source,
  IN  EFI_SYSTEM_CONTEXT          SystemContext     
  )

{
    disp_lcd_event_fun(1);
}

VOID EFIAPI disp_lcd2_event_proc(
  IN  HARDWARE_INTERRUPT_SOURCE   Source,
  IN  EFI_SYSTEM_CONTEXT          SystemContext     
  )

{
    disp_lcd_event_fun(2);
}

void disp_RegISR_ByScreenID(u32 irq_no, u32 screen_id)
{
    switch(screen_id)
    {
        case 0:
            OSAL_RegISR(irq_no,disp_lcd0_event_proc);
            break;
        case 1:
            OSAL_RegISR(irq_no,disp_lcd1_event_proc);
            break;
        case 2:
            OSAL_RegISR(irq_no,disp_lcd2_event_proc);
            break;
        default:
            DEBUG((DEBUG_ERROR,"disp_RegISR_ByScreenID: not support screen id:%d\n", screen_id));
    }
}
...
s32 disp_lcd_gpio_init(struct disp_lcd* lcd)
{
  ...
  for(i = 0; i < LCD_GPIO_REGU_NUM; i++)
  {
    if(!((!AsciiStrCmp(lcdp->lcd_cfg.lcd_gpio_regulator[i], "")) || (!AsciiStrCmp(lcdp->lcd_cfg.lcd_gpio_regulator[i], "none"))))
  ...
  for(i=0; i<LCD_GPIO_NUM; i++) {
    ...
    if(lcdp->lcd_cfg.lcd_gpio_used[i]) {
      ...
      CopyMem(gpio_info, &(lcdp->lcd_cfg.lcd_gpio[i]), sizeof(disp_gpio_set_t));
...
s32 disp_lcd_gpio_set_direction(struct disp_lcd* lcd, u32 io_index, u32 direction)
{
  ...
  AsciiSPrint(gpio_name,sizeof(gpio_name), "lcd_gpio_%d", io_index);
...
//      lcdp->pwm_info.dev = OSAL_Pwm_request(lcdp->panel_info.lcd_pwm_ch);
  ...
  if(disp_al_query_lcd_mod(lcd->channel_id)) {
    //OSAL_RegISR(lcdp->irq_no,0,disp_lcd_event_proc,(void*)lcd->channel_id,0,0);
        disp_RegISR_ByScreenID(lcdp->irq_no,lcd->channel_id);
#if !defined(__LINUX_PLAT__)

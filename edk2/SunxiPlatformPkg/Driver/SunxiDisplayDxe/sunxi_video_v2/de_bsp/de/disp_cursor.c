...
s32 disp_cursor_set_palette(struct disp_cursor* cursor, void *palette, u32 offset, u32 palette_size)
{
...
#if defined(__LINUX_PLAT__)
  {
    ...
    spin_lock_irqsave(&cursor_data_lock, flags);
#endif
    CopyMem((void*)&cursorp->info.palette[offset], palette, palette_size);
    cursorp->info_dirty_p = true;
#if defined(__LINUX_PLAT__)
    spin_unlock_irqrestore(&cursor_data_lock, flags);
  }
...
s32 disp_cursor_set_fb(struct disp_cursor* cursor, disp_cursor_fb *fb)
{
...
#if defined(__LINUX_PLAT__)
  {
    ...
    spin_lock_irqsave(&cursor_data_lock, flags);
#endif
    CopyMem(&cursorp->info.fb, fb, sizeof(disp_cursor_fb));
    cursorp->info_dirty = true;
#if defined(__LINUX_PLAT__)
    spin_unlock_irqrestore(&cursor_data_lock, flags);
  }
...
s32 disp_cursor_set_pos(struct disp_cursor* cursor, disp_position *pos)
{
...
#if defined(__LINUX_PLAT__)
  {
    ...
    spin_lock_irqsave(&cursor_data_lock, flags);
#endif
    CopyMem(&cursorp->info.pos, pos, sizeof(disp_position));
    cursorp->info_dirty = true;
#if defined(__LINUX_PLAT__)
    spin_unlock_irqrestore(&cursor_data_lock, flags);
  }
...
s32 disp_cursor_get_pos(struct disp_cursor* cursor, disp_position *pos)
{
...
#if defined(__LINUX_PLAT__)
  {
    ...
    spin_lock_irqsave(&cursor_data_lock, flags);
#endif
    CopyMem(pos, &cursorp->info.pos, sizeof(disp_position));
#if defined(__LINUX_PLAT__)
    spin_unlock_irqrestore(&cursor_data_lock, flags);
  }
#endif

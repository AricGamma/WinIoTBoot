...
//        __wrn("DEU_ALG, input width is ZERO\n");
  ...
  CopyMem(&frameinfo, &gdeu[sel].frameinfo, sizeof(disp_frame_info));
...
#endif
  CopyMem(g_strtab_addr, deu_str_tab, 512);
...
s32 IEP_Deu_Set_frameinfo(u32 sel, disp_frame_info frameinfo)
{
  ...
  CopyMem(&gdeu[sel].frameinfo, &frameinfo, sizeof(disp_frame_info));

...
s32 drc_init(u32 sel)
{
  ...
  SetMem(gpwrsv[sel].min_adj_index_hist,  sizeof(u8)*IEP_LH_PWRSV_NUM, 255);
...
static __inline s32 PWRSAVE_CORE(u32 sel)
{
  ...
  if(backlight < PWRSAVE_PROC_THRES) {
    ...
    if (abs((s32)min_adj_index - gpwrsv[sel].min_adj_index_hist[IEP_LH_PWRSV_NUM - 1]) > SCENE_CHNG_THR) {
      SetMem(gpwrsv[sel].min_adj_index_hist, sizeof(u8)*IEP_LH_PWRSV_NUM, min_adj_index);
    } else {
...
s32 IEP_Drc_Init(u32 sel)
{
  ...
  ZeroMem(&giep[sel],  sizeof(__drc_t));
  ZeroMem(&gpwrsv[sel],  sizeof(__drc_pwrsv_t));
...
//    __wrn("iep_drc_init, OSAL_PhyAlloc fail\n");
  ...
  AsciiSPrint(primary_key, sizeof(primary_key),"lcd%d_para", sel);
  ...
  CopyMem(pttab[sel], pwrsv_lgc_tab[128*lcdgamma], IEP_LGC_TAB_SIZE);
...
{
  CopyMem(&giep[sel].drc_win, &window, sizeof(disp_window));

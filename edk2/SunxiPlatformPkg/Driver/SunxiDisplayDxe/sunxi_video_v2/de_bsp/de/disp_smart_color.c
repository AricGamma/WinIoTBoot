...
s32 disp_smcl_update_regs(struct disp_smcl* smcl)
{
...
#if defined(__LINUX_PLAT__)
  {
    ...
    CopyMem(&smcl_info, &smclp->screen_info.info, sizeof(struct disp_smcl_info));
...
s32 disp_smcl_apply(struct disp_smcl* smcl)
{
...
#if defined(__LINUX_PLAT__)
  {
    ...
    if(smclp->screen_info.user_info_dirty) {
      CopyMem(&smclp->screen_info.info, &smclp->screen_info.user_info, sizeof(struct disp_smcl_info));
...
s32 disp_smcl_set_window(struct disp_smcl* smcl, disp_window *window)
{
...
#if defined(__LINUX_PLAT__)
  {
    ...
    spin_lock_irqsave(&smcl_data_lock, flags);
#endif
    CopyMem(&smclp->screen_info.user_info.window, window, sizeof(disp_window));
    smclp->screen_info.user_info_dirty = true;
#if defined(__LINUX_PLAT__)
    spin_unlock_irqrestore(&smcl_data_lock, flags);
  }

  ...
  )
{
  ...
  mLinuxKernelCurrentAtag->body.mem_tag.size  = 2048<<20;//Size;             /* Size of memory chunk for AtagMem */
  ...
  )
{
  ...
  script_parser_fetch("dram_para", "dram_para1", (INT32*)&DramSize, 1);
  DramSize = DramSize&0xffff;
  DramSize = DramSize<<20;
  DramBase = PcdGet64(PcdSystemMemoryBase);

  SetupMemTag(DramBase,DramSize);

...
int mmc_set_erase_start_addr(struct mmc *mmc, unsigned int address)
{
  ...
  if (err) {
    MMCINFO("%a: send erase start addr failed\n", __FUNCTION__);
    goto ERR_RET;
  }
...
int mmc_set_erase_end_addr(struct mmc *mmc, unsigned int address)
{
  ...
  err = mmc_send_cmd(mmc, &cmd, NULL);
  if (err) {
    MMCINFO("%a: send erase end addr failed\n", __FUNCTION__);
    goto ERR_RET;
  }
...
int mmc_do_sanitize(struct mmc *mmc)
{
  ...
  MMCINFO("%a: start emmc sanitize...\n", __FUNCTION__);
  ret = mmc_do_switch(mmc, EXT_CSD_CMD_SET_NORMAL,
          EXT_CSD_SANITIZE_START, 1,
          MMC_SANITIZE_REQ_TIMEOUT);
  MMCINFO("%a end emmc sanitzie, ret %d\n", __FUNCTION__, ret);
  return ret;
}
  ...
  unsigned int nr, unsigned int *skip_space, int secure)
{
  ...
  MMCDBG("%d %a: start erase, seucre %d...\n", __LINE__, __FUNCTION__, secure);
  ret = mmc_erase_group_aligned(mmc, from, nr);
  if (!ret) {
  ...
  unsigned int *skip_space)
{
  ...
  if (nr == 0) {
    MMCINFO("%a: on space need to erase, nr %d\n", __FUNCTION__, nr);
    return 0;
  }

  if (IS_SD(mmc)) {
    MMCINFO("%a: no mmc, do nothing\n", __FUNCTION__);
    ret = -2;
    goto ERR_RET;
  }
  ...
  unsigned int blkcnt, unsigned int *skip_space)
{
  ...
  MMCDBG("start %a ...\n", __FUNCTION__);
  if (IS_SD(mmc)) {
    MMCINFO("%a: sd card don't support erase\n", __FUNCTION__);
    ret = -1;
    goto ERR_RET;
  }

  if (mmc->cfg->platform_caps.drv_erase_feature & DRV_PARA_DISABLE_EMMC_ERASE) {
    MMCINFO("%a: driver don't support erase\n", __FUNCTION__);
    ret = -1;
    goto ERR_RET;
  }

  if (blkcnt == 0) {
    MMCINFO("%a: no space need to erase, from:%d nr:%d\n",
  ...
  if ((start+blkcnt) > mmc->block_dev.lba) {
    MMCINFO("%a: input lenght error!!!\n", __FUNCTION__);
    blkcnt = mmc->block_dev.lba - start;
    MMCINFO("%a: after clip, from: %d, nr: %d\n",
      __FUNCTION__, start, blkcnt);
  }

  ret = mmc_insecure_secure_erase(mmc, start, blkcnt, skip_space, 0);
  if (ret) {
    MMCINFO("%a: erase emmc fail!\n", __FUNCTION__);
  }

  if (skip_space[0]) {
    ret = 1;

    MMCINFO("%a: some sectors in emmc are ignored!\n", __FUNCTION__);
    for (i=0; i<2; i++)
      if (skip_space[0] & (1<<i))
    ...
    && (mmc->secure_feature & EXT_CSD_SEC_SANITIZE))
  {
    ...
    if (ret1) {
      MMCINFO("%a: emmc sanitize fail. ignore this error and continue...\n", __FUNCTION__);
    }
  }
...
int mmc_mmc_sanitize(int dev_num)
{
  ...
  if (!(mmc->secure_feature & EXT_CSD_SEC_SANITIZE)) {
    MMCINFO("%a: driver don't support sanitize\n", __FUNCTION__);
    ret = -1;
    goto ERR_RET;
  }
...
int mmc_mmc_trim(int dev_num, unsigned int start, unsigned int blkcnt)
{
  ...
  MMCDBG("start %a ...\n", __FUNCTION__);
  if (IS_SD(mmc)) {
    MMCINFO("%a: sd card don't support trim\n", __FUNCTION__);
    ret = -1;
    goto ERR_RET;
  }
...
int mmc_mmc_discard(int dev_num, unsigned int start, unsigned int blkcnt)
{
  ...
  MMCDBG("start %a ...\n", __FUNCTION__);
  if (IS_SD(mmc)) {
    MMCINFO("%a: sd card don't support discard\n", __FUNCTION__);
    ret = -1;
    goto ERR_RET;
  }
  ...
  if (mmc->version >= MMC_VERSION_4_5)
  {
    ...
    MMCINFO("%a: don't support discard!\n", __FUNCTION__);
    ret = -1;
  }
  ...
  unsigned int blkcnt, unsigned int *skip_space)
{
  ...
  MMCDBG("start %a ...\n", __FUNCTION__);
  if (IS_SD(mmc)) {
    MMCINFO("%a: sd card don't support secure erase!\n", __FUNCTION__);
    ret = -1;
    goto ERR_RET;
  }
  ...
  if (!(mmc->secure_feature & EXT_CSD_SEC_ER_EN)) {
    MMCINFO("%a: don't support secure erase!\n", __FUNCTION__);
    ret = -1;
    goto ERR_RET;
  }
  ...
  if (skip_space[0]) {
    MMCDBG("%a: some sectors in emmc are ignored!\n\n", __FUNCTION__);
    for (i=0; i<2; i++)
      if (skip_space[0] & (1<<i))
...
int mmc_mmc_secure_trim(int dev_num, unsigned int start, unsigned int blkcnt)
{
  ...
  MMCDBG("start %a ...\n", __FUNCTION__);
  if (IS_SD(mmc)) {
    MMCINFO("%a: sd card don't support secure trim!\n", __FUNCTION__);
    ret = -1;
    goto ERR_RET;
  }
    ...
    && (mmc->secure_feature & EXT_CSD_SEC_GB_CL_EN))
  {
    ...
    MMCINFO("%a: don't support secure trim!\n", __FUNCTION__);
    ret = -1;
  }

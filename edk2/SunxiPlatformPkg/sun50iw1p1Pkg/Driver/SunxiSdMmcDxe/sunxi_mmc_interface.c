...
static ulong mmc_do_burn_boot(int dev_num, lbaint_t start, lbaint_t blkcnt, const void*src)
{
  ...
  if ((rblk<blkcnt) || memcmp(src, dst, blkcnt * mmc->write_bl_len)) {
    MMCINFO("%a: check boot0 fail\n", __FUNCTION__);
    err = -1;
  } else
    MMCDBG("%a: check boot0 ok\n", __FUNCTION__);
...
static ulong mmc_burn_boot(int dev_num, lbaint_t start, lbaint_t blkcnt, const void*src)
{
  ...
  if ((mmc->cfg->platform_caps.drv_burn_boot_pos & DRV_PARA_BURN_EMMC_BOOT_PART)
    && mmc->boot_support)
  {
    MMCDBG("%a: start burn emmc boot part: %d...\n", __func__, (int)start);
    /* enable boot mode */
...
DISABLE_BOOT_MODE:
  ...
  {
    MMCDBG("%a: start burn emmc user part: %d...\n", __func__, (int)start);
...
static ulong mmc_get_boot(int dev_num, lbaint_t start, lbaint_t blkcnt, void *dst)
{
  ...
  if (!(mmc->cfg->platform_caps.drv_burn_boot_pos & DRV_PARA_NOT_BURN_USER_PART))
  {
    ...
    if (blocks_do != blkcnt) {
      MMCINFO("%a: get boot0 from user part err\n", __FUNCTION__);
    } else {
      MMCDBG("%a: get boot0 from user part ok, return..\n", __FUNCTION__);
      goto RET;
    }
  }
    ...
    && mmc->boot_support)
  {
    ...
    {
      MMCINFO("%a: switch to boot1 part failed\n", __FUNCTION__);
      goto RET;
    }
    ...
    */
    ...
    blocks_do = mmc_bread(dev_num, start_todo, blkcnt, dst);
    if (blocks_do != blkcnt) {
      MMCINFO("%a: get boot0 from boot part err\n", __FUNCTION__);
    } else {
      MMCDBG("%a: get boot0 from boot part ok\n", __FUNCTION__);
    }

    if (mmc_switch_part(dev_num, 0)) {
      MMCINFO("%a: switch back to user part failed,"
...
static ulong mmc_bread_mass_pro(int dev_num, ulong start, lbaint_t blkcnt, void *dst)
{
  ...
  if ((((dev_num == 2) &&(start == BOOT0_SDMMC_START_ADDR)) || ((dev_num == 3) && (start == BOOT0_EMMC3_START_ADDR)))
    && ((start+blkcnt) < CONFIG_MMC_LOGICAL_OFFSET))
  {
    MMCDBG("%a: start read boot data...\n", __FUNCTION__);
    return mmc_get_boot(dev_num, start, blkcnt, dst);
  }
  else /*if (start > CONFIG_MMC_LOGICAL_OFFSET)*/
  {
    MMCDBG("%a: start read user data...\n", __FUNCTION__);
...
mmc_bread_secure(int dev_num, ulong start, lbaint_t blkcnt, void *dst)
{
  if (check_secure_area(start,blkcnt)) {
    MMCINFO("Should not w/r secure area in fun %a,line,%d in start %ld,end %ld\n",\
...
{
  if (check_secure_area(start,blkcnt)) {
    MMCINFO("Should not w/r secure area in fun %a,line,%d in start %d,end %d\n",\
      __FUNCTION__,__LINE__,start,start + blkcnt-1);
    return -1;
  }
...
sdmmc_secure_storage_read(s32 dev_num,u32 item,u8 *buf , lbaint_t blkcnt)
{
  ...
  if(fst_bak == NULL){
    MMCINFO("malloc buff failed in fun %a line %d\n",__FUNCTION__,__LINE__);
    ret = -1;
    goto out;
  }
  ...
  if(ret != blkcnt){
    MMCINFO("read first backup failed in fun %a line %d\n",__FUNCTION__,__LINE__);
    ret = -1;
    goto out_sec;
  }
  /*second backups*/
  ...
  if(ret != blkcnt){
    MMCINFO("read second backup failed fun %a line %d\n",__FUNCTION__,__LINE__);
    ret = -1;
    goto out_sec;
  }

  if(memcmp(fst_bak,sec_bak,blkcnt*512)){
    MMCINFO("first and second bak compare failed fun %a line %d\n",__FUNCTION__,__LINE__);
    ret = -1;
  }else{

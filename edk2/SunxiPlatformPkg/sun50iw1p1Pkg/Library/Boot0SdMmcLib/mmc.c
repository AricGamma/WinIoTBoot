...
#include <Sun50iW1P1/sun50iw1p1.h>
...
int MmcSendCmd(struct mmc *mmc, struct mmc_cmd *cmd, struct mmc_data *data)
{
  return mmc->send_cmd(mmc, cmd, data);
}

int MmcSendStatus(struct mmc *mmc, int timeout)
{
  struct mmc_cmd cmd;
  int err;
...
struct mmc *FindMmcDevice(int dev_num)
{
...
{
  int err = 0;
  struct mmc *mmc = FindMmcDevice(dev_num);
...
MmcWriteBlocks(struct mmc *mmc, unsigned long start, unsigned blkcnt, const void*src)
{
  struct mmc_cmd cmd;
...
mmc_bwrite(int dev_num, unsigned long start, unsigned blkcnt, const void*src)
{
  ...
  if (MmcSetBlocklen(mmc, mmc->write_bl_len)){
    mmcinfo("mmc %d set block len failed\n",mmc->control_num);
//    sunxi_mmc_exit(dev_num);
//    if(sunxi_MmcInit(dev_num,4)<0){
...
//    }
  ...
  do {
    ...
    if(MmcWriteBlocks(mmc, start, cur, src) != cur){
...
int MmcReadBlocks(struct mmc *mmc, void *dst, unsigned long start, unsigned blkcnt)
{
  struct mmc_cmd cmd;
  struct mmc_data data;
  int timeout = 1000;
...
unsigned long
MmcBread(int dev_num, unsigned long start, unsigned blkcnt, void *dst)
{
  unsigned cur, blocks_todo = blkcnt;
  struct mmc *mmc = FindMmcDevice(dev_num);
  ...
  if (MmcSetBlocklen(mmc, mmc->read_bl_len)){
    mmcinfo("mmc %d Set block len failed\n",mmc->control_num);
    return 0;
  }
  ...
  do {
    cur = (blocks_todo > mmc->b_max) ?  mmc->b_max : blocks_todo;
    if(MmcReadBlocks(mmc, dst, start, cur) != cur){
...
int MmcSendOpCond(struct mmc *mmc)
{
  int timeout = 10000;
  struct mmc_cmd cmd;
  int err;
...
int MmcSendExtCsd(struct mmc *mmc, char *ext_csd)
{
  struct mmc_cmd cmd;
  struct mmc_data data;
  int err;
...
int MmcUpdatePhase(struct mmc *mmc)
{
  return mmc->update_phase(mmc);
}

int MmcSwitch(struct mmc *mmc, u8 set, u8 index, u8 value)
{
  struct mmc_cmd cmd;
  int timeout = 1000;
  int ret;
  ...
  ret = MmcUpdatePhase(mmc);
  if (ret) {
    mmcinfo("MmcSwitch: update clock failed after send cmd6\n");
    return ret;
  }
...
int MmcChangeFreq(struct mmc *mmc)
{
  char ext_csd[512];
  char cardtype;
  int err;
...
int MmcSwitchPart(int dev_num, unsigned int part_num)
{
  struct mmc *mmc = FindMmcDevice(dev_num);
  ...
  return MmcSwitch(mmc, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_PART_CONF,
        (mmc->part_config & ~PART_ACCESS_MASK)
        | (part_num & PART_ACCESS_MASK));
}

int SdSwitch(struct mmc *mmc, int mode, int group, u8 value, u8 *resp)
{
  struct mmc_cmd cmd;
  struct mmc_data data;
...
retry_scr:
  ...
  err = SdSwitch(mmc, SD_SWITCH_SWITCH, 0, 1, (u8 *)&switch_status);
...
void MmcSetClock(struct mmc *mmc, u32 clock)
{
  if (clock > mmc->f_max)
    clock = mmc->f_max;
...
void MmcSetBusWidth(struct mmc *mmc, u32 width)
{
  mmc->bus_width = width;
...
int MmcStartup(struct mmc *mmc)
{
  ...
  MmcSetClock(mmc, mmc->tran_speed);
...
int MmcInit(struct mmc *mmc)
{
  ...
  err = MmcSendIfCond(mmc);

  /* Now try to get the SD card's operating condition */
  err = SdSendOpCond(mmc);

  /* If the command timed out, we check for an MMC card */
  if(err){
    mmcinfo("***Try MMC card %d***\n",mmc->control_num);
    err = MmcSendOpCond(mmc);
...
int MmcRegister(int dev_num, struct mmc *mmc)
{

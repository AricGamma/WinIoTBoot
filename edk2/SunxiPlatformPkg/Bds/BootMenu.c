  ...
  )
{
  ...
  CHAR16 Args[64];
    ...
    // Select which entry to call
  while(1){
      Print(L"Boot Devices Choice:\n");
      Print(L"[0] nand\n");
      Print(L"[1] card0\n");
      Print(L"[2] card2\n");
      Print(L"[3] Return to main menu\n");
      Print(L"Choice:");
      Status = GetHIInputInteger (&StorageType);
      if (EFI_ERROR(Status) || (StorageType == 3)) {
            Print(L"\n");
            return EFI_SUCCESS;
      }
  ...
  StrCpy(Args, L"BootArm.efi ");
  if(StorageType == 0) { StrCat(Args, L"nand");}
  if(StorageType == 1) { StrCat(Args, L"card0");}
  if(StorageType == 2) { StrCat(Args, L"card2");}
  // Start EFI Shell
  Status = BdsLoadApplication (mImageHandle, L"BootArm", StrLen(Args)+sizeof(CHAR16), (VOID*)Args);
  if (Status == EFI_NOT_FOUND) {
    Print (L"Error: EFI Application not found.\n");
  } else if (EFI_ERROR(Status)) {
    Print (L"Error: Status Code: 0x%X\n",(UINT32)Status);
...
#else
  EFI_STATUS Status;
  Status = BdsLoadApplication (mImageHandle, L"BootArm", 0, NULL);
  if (Status == EFI_NOT_FOUND) 
  {
    Print (L"Error: EFI Application not found.\n");
  } 
  ...
  )
{
  ...
  // Start EFI Shell
  Status = BdsLoadApplication (mImageHandle, L"Efex", 0, NULL);
  if (Status == EFI_NOT_FOUND) {
    Print (L"Error: EFI Application not found.\n");
  } else if (EFI_ERROR(Status)) {
    Print (L"Error: Status Code: 0x%X\n",(UINT32)Status);
  ...
  )
{
  ...
  // Start EFI Shell
  Status = BdsLoadApplication (mImageHandle, L"Fastboot", 0, NULL);
  if (Status == EFI_NOT_FOUND) {
    Print (L"Error: EFI Application not found.\n");
  } else if (EFI_ERROR(Status)) {
    Print (L"Error: Status Code: 0x%X\n",(UINT32)Status);
  ...
  )
{
  ...
  // Start EFI Shell
  Status = BdsLoadApplication (mImageHandle, L"CardBurn", 0, NULL);
  if (Status == EFI_NOT_FOUND) {
    Print (L"Error: EFI Application not found.\n");
  } else if (EFI_ERROR(Status)) {
    Print (L"Error: Status Code: 0x%X\n",(UINT32)Status);
  ...
  )
{
  ...
  // Start EFI Shell
  Status = BdsLoadApplication (mImageHandle, L"PrivateBurn", 0, NULL);
  if (Status == EFI_NOT_FOUND) {
    Print (L"Error: EFI Application not found.\n");
  } else if (EFI_ERROR(Status)) {
    Print (L"Error: Status Code: 0x%X\n",(UINT32)Status);
  ...
  )
{
  ...
  // Start EFI Shell
  Status = BdsLoadApplication (mImageHandle, L"UsbMass", 0, NULL);
  if (Status == EFI_NOT_FOUND) {
    Print (L"Error: EFI Application not found.\n");
  } else if (EFI_ERROR(Status)) {
    Print (L"Error: Status Code: 0x%X\n",(UINT32)Status);
...
/* for test application, your can change the test item */
EFI_STATUS
BootTestApp(
  IN LIST_ENTRY *BootOptionsList
  )
{
  ...
  // Start EFI Shell
  Status = BdsLoadApplication (mImageHandle, L"SdMmcTest", 0, NULL);
  if (Status == EFI_NOT_FOUND) {
    Print (L"Error: EFI Application not found.\n");
  } else if (EFI_ERROR(Status)) {
    Print (L"Error: Status Code: 0x%X\n",(UINT32)Status);
...
struct BOOT_MAIN_ENTRY {
  ...
  EFI_STATUS (*Callback) (IN LIST_ENTRY *BootOptionsList);
    ...
    { L"Boot Android", BootLinux },
    { L"Boot Efex",    BootEfexAfterReset },
    { L"Boot Fastboot",BootFastboot },
    //{ L"Boot CardBurn",BootCardBurn },
    { L"Boot PrivateBurn",BootPrivateBurn },
    { L"Boot UsbMass",BootUsbMass },
    /* this item for tese application */
    { L"Boot TestApp",BootTestApp },
};
